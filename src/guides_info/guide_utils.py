...
def exon_features_from_gtf(exon_list, exon_dict):
    if len(exon_list) == 0:  # no protein coding trsnacript
        ...
        for exon in exon_list:
            ...
            if exon_biotype == "coding":
                ...
                total_no_exon_transcript_list_all = total_no_exon_transcript_list_all + ast.literal_eval(
                    exon_feautre[2])
    ...
    return (
        total_transcipt_count, str(transcript_list_all), str(total_no_exon_transcript_list_all),
        str(exon_rank_list_all))
    ...
    if guide_exon in exon_dict.exon_transcript_info:
        ...
        if "nan" in exon_cds_start_stop_list:
            ...
            if exon_strand == "-":  ####### real cds start is stop according to annotation
                dist_cds_start_cutsite_list_coding = [abs(cutsite18 - x) for x in exon_cds_stop_list]
                dist_cds_stop_cutsite_list_coding = [abs(cutsite18 - x) for x in exon_cds_start_list]

            if exon_strand == "+":
                dist_cds_start_cutsite_list_coding = [abs(cutsite18 - x) for x in exon_cds_start_list]
                dist_cds_stop_cutsite_list_coding = [abs(cutsite18 - x) for x in exon_cds_stop_list]
...
def cutsite_distance_fom_exon_cds(exon_list, cutsite18, exon_dict):
    if len(exon_list) == 0:
